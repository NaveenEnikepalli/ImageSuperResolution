"""SSIM metric wrapper around PyTorch Image Quality (PIQ).

Author: Antigravity
Purpose: Wrap PIQ SSIM calculation. Tensors are clamped to range [0.0, 1.0]
...
class SSIMMetric:
    """Structural Similarity (SSIM) metric calculator wrapper.
    ...
    def __call__(self, prediction: torch.Tensor, target: torch.Tensor) -> float:
        ...
        score = piq.ssim(pred, tgt, data_range=1.0)
